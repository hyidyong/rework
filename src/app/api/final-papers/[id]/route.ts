...
export async function GET(_request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const { data, error } = await admin
    .from("final_papers")
    .select("id, owner_id, proposal_id, version, title, body_ciphertext, body_iv, body_tag")
    .eq("id", id)
    .eq("owner_id", authData.user.id)
    .single();
  if (error || !data) return Response.json({ error: "Final paper not found" }, { status: 404 });
  const row = data as FinalPaperRow;
  const env = readServerEnv();
  const markdown = decryptText(
    { version: 1, algorithm: "aes-256-gcm", ciphertext: row.body_ciphertext, iv: row.body_iv, tag: row.body_tag },
    env.DATA_ENCRYPTION_KEY,
    `${row.owner_id}:${row.proposal_id}:final:${row.version}`,
  );
  const filename = row.title.replace(/[^\p{L}\p{N}._-]+/gu, "_").slice(0, 100) || "paper";
