...
function assertAllowedHttpsUrl(
  ...
  if (!allowlist.has(url.hostname))
    throw new Error(`Skill host is not allowlisted: ${url.hostname}`);
  if (url.username || url.password)
    throw new Error("Credential-bearing skill URLs are forbidden");
  return url;
}
...
export async function loadRemoteSkill(
  ...
  if (digest !== manifest.sha256)
    throw new Error("Skill prompt digest does not match its manifest");
