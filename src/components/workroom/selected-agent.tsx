...
export function SelectedAgent({ agent }: { agent: AgentView }) {
  return (
    <section
      className="panel selected-agent"
      aria-labelledby="selected-agent-title"
    >
      <div className="panel-heading">
        <div>
          <p className="eyebrow">INSPECTOR</p>
          <h2 id="selected-agent-title">선택된 에이전트</h2>
        </div>
      </div>
      <div className="agent-profile">
        <span className="profile-orbit" data-state={agent.state}>
          <Activity size={26} />
        </span>
        <div>
          <strong>{agent.name}</strong>
          <p>
            <i className="state-dot" data-state={agent.state} />
            {agent.state}
          </p>
        </div>
      </div>
      <dl className="agent-details">
        ...
        <div>
          <dt>
            <ShieldCheck size={14} /> 보안
          </dt>
          <dd>원문 미노출 · 요약 이벤트만 전송</dd>
        </div>
        <div>
          <dt>
            <FileText size={14} /> 진행률
          </dt>
          <dd>
