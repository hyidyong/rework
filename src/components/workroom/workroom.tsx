...
export function Workroom({ supabaseConfig }: { supabaseConfig: PublicEnv }) {
  ...
  const defaultRole =
    scenario.agents.find(
      (agent) => agent.state !== "idle" && agent.state !== "completed",
    )?.role ?? "analyzer";
  const [selectedRole, setSelectedRole] = useState<AgentRole>(defaultRole);
  const selectedAgent = useMemo(
    () =>
      scenario.agents.find((agent) => agent.role === selectedRole) ??
      scenario.agents[0],
  ...
  return (
    <div className="app-shell" id="top">
      <Sidebar />
      <main ref={workroomRef} className="workroom" id="dashboard">
        <header className="topbar">
          <div>
            <p className="top-kicker">ACADEMIC MULTI-AGENT SYSTEM</p>
            <h1>논문 작성 워룸</h1>
          </div>
          <button className="project-switcher" type="button">
            <FolderOpen size={17} />
            <span>프로젝트: {scenario.projectTitle}</span>
            <ChevronDown size={15} />
          </button>
        ...
        <section
          ...
          aria-label={`전체 진행률 ${scenario.overallProgress}%`}
        >
          <strong>전체 진행률</strong>
          <b>{scenario.overallProgress}%</b>
          <span className="overall-progress">
            <i style={{ width: `${scenario.overallProgress}%` }} />
          </span>
          ...
          <span className="mode-badge" data-mode={scenario.connection}>
            {scenario.connection === "live"
              ? "LIVE DATA"
        ...
        <div className="workspace-grid">
          ...
          <div className="lower-workspace" id="logs">
            <LiveLogs logs={scenario.logs} />
            <SelectedAgent agent={selectedAgent} />
          </div>
          <div id="literature">
            <LiteratureMap
              nodes={scenario.literature}
        ...
        <footer className="app-footer">
          ...
          <span>Encrypted at rest · RLS · Realtime</span>
        </footer>
      </main>
    </div>
  );
